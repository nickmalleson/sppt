...
#r <- msea(base.data, test.data, N=5, n.shifts = 2, ignore.zeros = TRUE)
...
lo <- if (USE.LM) lo <- lm(y ~ x + I(x^2) ) else loess(y~x, degree = DEGREE)
...
# Calculate gradient at each x
testfun <- function(x) coef(lo)[1] + (coef(lo)[2]*x) + (coef(lo)[3]*(x^2))
stopifnot(! (FALSE %in% predict(lo) == testfun(x)))
d <- Deriv( f = testfun, x="x" )
