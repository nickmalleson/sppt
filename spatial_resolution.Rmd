...
```{r model_and_elbow, fig.width=9, fig.height=7 }
...
do.plot <- function(i) {
  ...
  elbox.index <- min(which(grads.log1 < 0.00002)) # The index at which the elbow crosses the x axis
  ...
  # Plot the tangent 
  yint <- elbow.y - (d.log1(elbow)*elbow) # Y intercept of the tangent
  abline(yint, d.log1(elbow), lty="dashed", lwd=2) # The gradient is  d.log1(elbow)
  
  # Add the model
  lines(x=x, y=predict(m.log ), col=1, lwd=2)
  
  # Add vertical and horizontal line at the elbow and a point
  abline(v=elbow,   col="red", lty="dashed")
  abline(h=elbow.y, col="red", lty="dashed")
  points(x=elbow, y=elbow.y, col="blue", pch=16, cex=2)
  
  # Add the equation to the graph
  mtext(paste0("y = ",round(alpha,3)," + ",round(beta,3),"*ln(x)","\n",
               "elbow (x)"," = ", round(elbow,5),"\n",
               "elbow (y)"," = ", round(elbow.y,5)
