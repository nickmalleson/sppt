...
bner.result <- msea(points1 = p1, points2 = p2,
                    N=200, n.shifts=3, ignore.zeros = F, step=10,
                    return.sobject=TRUE,
                    return.grids=TRUE)
...
(_Not running, takes ages, do it outside of this Rmd. See script and associated data `large_iteration_test.R*`_)

```{r runMSEA-test, eval=FALSE}
p1 <- BNER[BNER$YEAR==2015,]
p2 <- BNER[BNER$YEAR==2016,]
...
```{r runMSEA-test-plot, eval=FALSE}

error.stats <- c("rss", "r.squared", "rmse", "globalS", "globalS.robust") # The error stats. to use
par(mfrow=c(2,length(error.stats))) # One plot for each error statisticm using both num.cells and square area
...
for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
     ...
     plot(x,y, main=paste0(name,", ",e), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}
...
```{r s.index-comparisons1, fig.width=9, fig.height=6 }

options(scipen=1) # A bit of scientific notation is OK
par(mfrow=c(3,length(all))) # One plot for each S index

# Make a list so we can loop over the different tests
s.indices <- list(
  "output"=list(all, all.boot, all.diff),
  "name" = list("Normal", "Bootstrap", "Proportional Difference"),
  "cols"=list("#fc8d59","#ffffbf","#99d594") # Colours used later
  )

for (i in 1:length(s.indices[["output"]])) { # For each of the three tests
  for (a in s.indices[["output"]][[i]]) { # for each crime type in this test
    r <- a$r # This is the results of the test
    crime.type <- a$name # This is the name of the crime type
    error.type <- s.indices[["name"]][[i]] # The kind of S Index
     x <- r$num.cells
     y <- r[["globalS"]] # The S Index (either normal, boot, or prop)
     plot(x,y, main=paste0(error.type,": ",crime.type), 
          xlab="Number of Cells", ylab=paste0(error.type), ylim=c(0,1))
...
Same again but all on the same graph _TODO and also show square area as well as num cells_
...
for (i in 1:length(crime.types)) { # For each crime type
  ...
  for (j in 1:length(test.results)) { # For each test result
    ...
    y <- r[["globalS"]] # The S Index (either normal, boot, or prop)
    if (j==1) {
      plot(x,y, main=paste0(crime.type), xlab="Number of Cells", ylab="Similarity", ylim=c(0,1), col=colour, pch=1)
...
for (i in 1:length(crime.types)) { # For each crime type
  ...
  for (j in 1:length(test.results)) { # For each test result
    ...
    y <- r[["globalS"]] # The S Index (either normal, boot, or prop)
    if (j==1) {
      plot(x,y, main=paste0(crime.type), xlab="Square area of cells", 
           ylab="Similarity", ylim=c(0.4,1), xlim=c(0,80000), col=colour, pch=1)
      legend("bottomright", legend = names, col = cols, pch=1)
    } else {
      points(x,y, col=colour, pch=1)
    }
