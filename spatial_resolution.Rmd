...
Recent research has, almost unanimously.
XXXX
...
Now run the function. Just use two years of burglary data for now

```{r runMSEA, cache=TRUE }
base.data <- BNER[BNER$YEAR==2015,]
test.data <- BNER[BNER$YEAR==2016,]

#r <- msea(base.data, test.data, N=5, n.shifts = 2, ignore.zeros = FALSE)

#r <-           msea(base.data, test.data, N=120, n.shifts=5, ignore.zeros = T, step=2)
#r.withzeros <- msea(base.data, test.data, N=120, n.shifts=5, ignore.zeros = F, step=2)

# Do for both with and without zeros, at the same time
tmp <- mclapply(X = c(T,F), FUN = function(x) {
  msea(base.data, test.data, N=150, n.shifts=5, ignore.zeros = x, step=5)
})
r <- tmp[[1]]
r.withzeros <- tmp[[2]]
rm(tmp)
...
```{r map.totals , fig.width=9, fig.height=4}
par(mfrow=c(2,4))
...
```{r map.abs.prop.diff.fuzzy, fig.width=6, fig.height=5}
...
results.to.map <- NA
if (length(r$results) > 50) {
  results.to.map <- sample(x=1:length(r$results), size=50, replace = FALSE )
  print("Only mapping 50 of all grids ")
} else {
  results.to.map <- 1:(length(r$results)-1)
}
...
s         <- shading(s.breaks, cols=add.alpha(brewer.pal(length(s.breaks)+1,'RdYlGn'), (1/length(results.to.map))))
...
Also do this for the results with and without zeros

Note: _the data have been reversed for the square area of a cell graphs (so points with the smallest cells occur on the right of the graph), but the axis label hasn't been reversed so is wrong. (This will only matter for publication)_

```{r graphError, fig.width=9, fig.height=12 }
...
# RSS
plot(x=r$num.cells, y=r$rss, type='o', main="RSS (no zeros)", xlab="Number of cells")
plot(x=rev(r$cell.areas), y=r$rss, type='o', main="RSS (no zeros)", xlab="Square area of a cell")
plot(x=r.withzeros$num.cells, y=r.withzeros$rss, type='o', main="RSS (with zeros)", xlab="Number of cells")
plot(x=rev(r.withzeros$cell.areas), y=r.withzeros$rss, type='o', main="RSS (with zeros)", xlab="Square area of a cell")

# RSquared
plot(x=r$num.cells, y=r$r.squared, type='o', main="R-Squared (no zeros)", xlab="Number of cells")
plot(x=rev(r$cell.areas), y=r$r.squared, type='o', main="R-Squared (no zeros)", xlab="Square area of a cell")
plot(x=r.withzeros$num.cells, y=r.withzeros$r.squared, type='o', main="R-Squared (with zeros)", xlab="Number of cells")
plot(x=rev(r.withzeros$cell.areas), y=r.withzeros$r.squared, type='o', main="R-Squared (with zeros)", xlab="Square area of a cell")

# RMSE
plot(x=r$num.cells, y=r$rmse, type='o', main="RMSE (no zeros)", xlab="Number of cells")
plot(x=rev(r$cell.areas), y=r$rmse, type='o', main="RMSE (no zeros)", xlab="Square area of a cell")
plot(x=r.withzeros$num.cells, y=r.withzeros$rmse, type='o', main="RMSE (with zeros)", xlab="Number of cells")
plot(x=rev(r.withzeros$cell.areas), y=r.withzeros$rmse, type='o', main="RMSE (with zeros)", xlab="Square area of a cell")

# Global S
plot(x=r$num.cells, y=r$globalS, type='o', main="Global S (no zeros)", xlab="Number of cells")
plot(x=rev(r$cell.areas), y=r$globalS, type='o', main="Global S (no zeros)", xlab="Square area of a cell")
plot(x=r.withzeros$num.cells, y=r.withzeros$globalS, type='o', main="Global S (with zeros)", xlab="Number of cells")
plot(x=rev(r.withzeros$cell.areas), y=r.withzeros$globalS, type='o', main="Global S (with zeros)", xlab="Square area of a cell")

# Global S (Robust)
plot(x=r$num.cells, y=r$globalS.robust, type='o', main="Global S (robust)\n(no zeros)", xlab="Number of cells")
plot(x=rev(r$cell.areas), y=r$globalS.robust, type='o', main="Global S (robust)\n(no zeros)", xlab="Square area of a cell")
plot(x=r.withzeros$num.cells, y=r.withzeros$globalS.robust, type='o', main="Global S (robust)\n(with zeros)", xlab="Number of cells")
plot(x=rev(r.withzeros$cell.areas), y=r.withzeros$globalS.robust, type='o', main="Global S (robust)\n(with zeros)", xlab="Square area of a cell")


```

**TODO: discuss these results. Why do RSS and RMSE decrese? Errors should increase?**

Now estimate a quadratic model that represents the change in S Index. Later this will be differentiated to calculate the point at which the gradient is 45 degrees.

_Good information about fitting polynomials in R_ [here](https://www.r-bloggers.com/fitting-polynomial-regression-in-r/).

```{r s.model, fig.width=9, fig.height=6 }
par(mfrow=c(2,4))

START <-10 # Sometimes ignore first few grids
END <- length(r$cell.areas)

FORM <- y ~ x + I(x^2) + I(x^3)

# Calculate the gradient of a model, m, at each point, x (assumes m is a polynomial)
calc.gradients <- function(m, x) {
  f <- function(x) coef(m)[1] + (coef(m)[2]*x) + (coef(m)[3]*(x^2)) #+ (coef(m)[4]*(x^3)) 
  stopifnot(! (FALSE %in% predict(m) == f(x))) # Check function has been specified correctly
  d <- Deriv( f = f, x="x", nderiv = 1 ) # Calculate the second derivitive
  gradients <- vapply(X = x, FUN=d, FUN.VALUE = numeric(1))
  return(gradients)
}

make.gradient.plot <- function(x, y, form, main="") {
  m <- lm( form, data=list("x"=x, "y"=y) ) 
  plot(x,y, main=main )
  lines(x=x, y=predict(m), col='red', lwd=2)
  gradients <- calc.gradients(m,x)
  lines(abline(v=x[which.min(abs(gradients - 0))], col="blue", lty=2)) # Point at which the gradient is closest to 0
  lines(abline(h=y[which.min(abs(gradients - 0))], col="blue", lty=2))
  # Can show the gradient on the same chart (useful for debugging)
  points(x=x, y=((gradients*1000)+0.3), col=sapply(X=gradients, FUN=function(z) if(z>0) "blue" else "red"))
  #print(gradients)
}


# START BY INCLUDING ZEROS

make.gradient.plot( x=r.withzeros$num.cells[START:END], 
                    y=r.withzeros$globalS.robust[START:END],
  form=FORM, main = "NumCells : Robust :\nZeros" )

make.gradient.plot( x=r.withzeros$num.cells[START:END], 
                    y=r.withzeros$globalS[START:END],
  form=FORM, main = "NumCells : Normal :\nZeros" )

make.gradient.plot( x=r.withzeros$cell.areas[START:END], 
                    y=r.withzeros$globalS.robust[START:END],
  form=FORM, main = "CellAreas : Robust :\nZeros" )

make.gradient.plot( x=r.withzeros$cell.areas[START:END], 
                    y=r.withzeros$globalS[START:END],
  form=FORM, main = "CellAreas : Normal :\nZeros" )
...
make.gradient.plot( x=r$num.cells[START:END], 
                    y=r$globalS.robust[START:END],
  form=FORM, main = "NumCells : Robust :\nNo Zeros" )
...
make.gradient.plot( x=r$cell.areas[START:END], 
                    y=r$globalS.robust[START:END],
  form=FORM, main = "CellAreas : Robust :\nNo Zeros" )
...
Interestingly the graphs with number of cells on the x axis all reach their elbow at the same resolution, albeit with different `globalS` values, regardless of the choice of statistic (normal or robust) or the inclusion (or not) of cells with no crimes in.

Repeat above, but this time look at the spatial error resolution of a few different crime types
...
# Do all at the same time. Include zeros.
tmp <- mclapply(X = all, FUN = function(x) {
  #msea(x[['base']], x[['test']], N=10, n.shifts=1, ignore.zeros = F, step=1)
  msea(x[['base']], x[['test']], N=150, n.shifts=5, ignore.zeros = F, step=5,
       return.sobject=FALSE, return.grids=FALSE)
...
Now graph the change in local S index for each crime type.)

```{r s.model-all, fig.width=9, fig.height=6}
par(mfrow=c(length(all)/2,4))
FORM <- y ~ x + I(x^2) + I(x^3) # Cubic formula to fit to change in S index
...
for (a in all) {
  ...
  # This code to graph and calculate gradients came from the 's.model' chunk.
  # I've expanded the functions here to make it easier to play around with
  
  col1 <- "#d8b365"
  col2 <- "#5ab4ac"
  
  x =        r$num.cells
  y.robust = r$globalS.robust
  y.normal = r$globalS
  
  # Plot points
  plot(  x, y.normal, main=name, pch=1, ylim=c(0,1), col=col1, 
         xlab="Number of Cells", ylab="Global S Index")
  points(x, y.robust, pch=2, col=col2)
  
  # Calculate model for points using FORM
  m.normal <- lm( FORM, data=list("x"=x, "y"=y.normal) ) 
  m.robust <- lm( FORM, data=list("x"=x, "y"=y.robust) ) 
  lines(x=x, y=predict(m.normal), col="red", lwd=2)
  lines(x=x, y=predict(m.robust), col="red", lwd=2)
  
  # Calculate gradients
  
  # Function to differentiate. Do two versions - the proper version, and one where we ignore the cubic term
  FUNC.normal1 <- function(x) 
    coef(m.normal)[1] + (coef(m.normal)[2]*x) + (coef(m.normal)[3]*(x^2)) + (coef(m.normal)[4]*(x^3)) 
  FUNC.robust1 <- function(x) 
    coef(m.robust)[1] + (coef(m.robust)[2]*x) + (coef(m.robust)[3]*(x^2)) + (coef(m.robust)[4]*(x^3)) 
  FUNC.normal2 <- function(x) coef(m.normal)[1] + (coef(m.normal)[2]*x) + (coef(m.normal)[3]*(x^2))
  FUNC.robust2 <- function(x) coef(m.robust)[1] + (coef(m.robust)[2]*x) + (coef(m.robust)[3]*(x^2))
  
  stopifnot(! (FALSE %in% (predict(m.normal) == FUNC.normal1(x)))) # Check function has been specified correctly
  stopifnot(! (FALSE %in% (predict(m.robust) == FUNC.robust1(x)))) 
  
  # Differentiate the functions
  d.normal1 <- Deriv( f = FUNC.normal1, x="x", nderiv = 1 )
  d.robust1 <- Deriv( f = FUNC.robust1, x="x", nderiv = 1 ) 
  d.normal2 <- Deriv( f = FUNC.normal2, x="x", nderiv = 1 )
  d.robust2 <- Deriv( f = FUNC.robust2, x="x", nderiv = 1 ) 
  grads.normal1 <- vapply(X = x, FUN=d.normal1, FUN.VALUE = numeric(1))
  grads.robust1 <- vapply(X = x, FUN=d.normal1, FUN.VALUE = numeric(1))
  grads.normal2 <- vapply(X = x, FUN=d.normal2, FUN.VALUE = numeric(1))
  grads.robust2 <- vapply(X = x, FUN=d.normal2, FUN.VALUE = numeric(1))
  
  # Mark the points of the elbow (when the gradient is closest to zero)
  
  lines(abline(v=x[which.min(abs(grads.normal1 - 0))], col=col1, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.robust1 - 0))], col=col2, lty=3, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.normal2 - 0))], col=col1, lty=4, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.robust2 - 0))], col=col2, lty=5, lwd=2)) # Point at which the gradient is closest to 0
  
  # Also plot the gradients as a separate graph
  plot(   x=x, y=grads.normal1, col="red", pch=1, ylab="Gradient", 
          ylim=c(min(grads.normal1, grads.normal2),max(grads.normal1, grads.normal2)))
  #points( x=x, y=grads.robust1, col=col2, pch=2)
  points( x=x, y=grads.normal2, col="blue", pch=1)
  #points( x=x, y=grads.robust2, col=col2, pch=2)
...
Repeat but don't bother with 'robust' versions as the gradients are the same


```{r s.model-all2, fig.width=9, fig.height=6 }
options(scipen=100) # Less scientific notation please
par(mfrow=c(length(all)/2,4))
FORM.CUBE <- y ~ x + I(x^2) + I(x^3) # Cubic formula to fit to change in S index
FORM.QUAD <- y ~ x + I(x^2) # Quadratic
...
for (a in all) {
  ...
  # Plot points
  plot(  x, y.normal, main=name, ylim=c(0,1), col="#999999", pch='o', 
         xlab="Number of Cells", ylab="Global S Index")
  
  # Calculate models for points using FORM
  
  m.quad <- lm( FORM.QUAD, data=list("x"=x, "y"=y.normal) ) 
  m.cube <- lm( FORM.CUBE, data=list("x"=x, "y"=y.normal) ) 
  lines(x=x, y=predict(m.quad), col=col1, lwd=2)
  lines(x=x, y=predict(m.cube), col=col2, lwd=2)
  
  # Calculate gradients
  
  # Function to differentiate. Do three versions - the quad, the cubic, and the weird one (cubic without x^3 term)
  FUNC.cube <- function(x) coef(m.cube)[1] + (coef(m.cube)[2]*x) + (coef(m.cube)[3]*(x^2)) + (coef(m.cube)[4]*(x^3)) 
  
  FUNC.quad <- function(x) coef(m.quad)[1] + (coef(m.quad)[2]*x) + (coef(m.quad)[3]*(x^2))
  
  FUNC.weird <- function(x) coef(m.cube)[1] + (coef(m.cube)[2]*x) + (coef(m.cube)[3]*(x^2))
  
  stopifnot(! (FALSE %in% (predict(m.quad) == FUNC.quad(x)))) # Check function has been specified correctly
  stopifnot(! (FALSE %in% (predict(m.cube) == FUNC.cube(x)))) # Check function has been specified correctly
  
  # Differentiate the functions
  d.quad  <- Deriv( f = FUNC.quad,  x="x", nderiv = 1 )
  d.cube  <- Deriv( f = FUNC.cube,  x="x", nderiv = 1 )
  d.cube2 <- Deriv( f = FUNC.cube,  x="x", nderiv = 2 ) # Try second derivative of cube as well
  d.weird <- Deriv( f = FUNC.weird, x="x", nderiv = 1 )
  
  grads.quad   <- vapply(X = x, FUN=d.quad,  FUN.VALUE = numeric(1))
  grads.cube   <- vapply(X = x, FUN=d.cube,  FUN.VALUE = numeric(1))
  grads.cube2  <- vapply(X = x, FUN=d.cube2, FUN.VALUE = numeric(1))
  grads.weird  <- vapply(X = x, FUN=d.weird, FUN.VALUE = numeric(1))
  
    # Mark the points of the elbow (when the gradient is closest to zero)
  
  lines(abline(v=x[which.min(abs(grads.quad  - 0))], col=1, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.cube  - 0))], col=2, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.cube2 - 0))], col=3, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.weird - 0))], col=4, lty=2, lwd=2)) # Point at which the gradient is closest to 0


  # Also plot the gradients as a separate graph
  plot(   x=x, y=grads.quad,  col=1, pch=1, ylab="Gradient", main="Gradients",
          ylim=c(min(grads.quad, grads.cube, grads.cube2, grads.weird),max(grads.quad, grads.cube, grads.cube2, grads.weird)))
  points( x=x, y=grads.cube,  col=2, pch=1)
  points( x=x, y=grads.cube2, col=3, pch=1)
  points( x=x, y=grads.weird, col=4, pch=1)
  ...
  legend("bottomleft", legend=c("Quad","Cube","Cube2","Weird"), pch="o", col=c(1,2,3,4))
  
}

```

These polynomials don't work so well for crimes that are more heavily clustered than residential burglary. Try some other functions: **log and sqrt**

```{r s.model-all3, fig.width=9, fig.height=6 }
options(scipen=100) # Less scientific notation please
par(mfrow=c(length(all)/2,4))

FORM.LOG  <- y ~ log(x)
FORM.SQRT <- y ~ sqrt(x)
FORM.CUBE <- y ~ x + I(x^2) + I(x^3) # Keep cubic for comparison
...
for (a in all) {
  ...
  m.cube <- lm( FORM.CUBE, data=list("x"=x, "y"=y.normal) ) 
  m.sqrt <- lm( FORM.SQRT, data=list("x"=x, "y"=y.normal) ) 
  m.log  <- lm( FORM.LOG,  data=list("x"=x, "y"=y.normal) )
  lines(x=x, y=predict(m.cube), col=col1, lwd=2)
  lines(x=x, y=predict(m.sqrt), col=col2, lwd=2)
  lines(x=x, y=predict(m.log ), col=col3, lwd=2)
  
  # Check that they have the extected number of coefficients
  stopifnot(length(coef(m.cube))==4)
  stopifnot(length(coef(m.sqrt))==2)
  ...
  FUNC.sqrt <- function(x) coef(m.sqrt)[1] + (coef(m.sqrt)[2]*sqrt(x) )
  

  #a <- data.frame(list("x"=x, "model"=predict(m.sqrt), "function"=FUNC.sqrt(x)) )
  
  stopifnot(! (FALSE %in% (predict(m.cube) == FUNC.cube(x)))) # Check function has been specified correctly
  stopifnot(! (FALSE %in% (predict(m.sqrt) == FUNC.sqrt(x)))) # Check function has been specified correctly
  stopifnot(! (FALSE %in% (predict(m.log ) == FUNC.log (x)))) # Check function has been specified correctly
  
  # Differentiate the functions
  d.cube  <- Deriv( f = FUNC.cube, x="x", nderiv = 1 )
  d.sqrt  <- Deriv( f = FUNC.sqrt, x="x", nderiv = 1 )
  d.log   <- Deriv( f = FUNC.log,  x="x", nderiv = 1 )
  
  grads.cube <- vapply(X = x, FUN=d.cube,  FUN.VALUE = numeric(1))
  grads.sqrt <- vapply(X = x, FUN=d.sqrt,  FUN.VALUE = numeric(1))
  grads.log  <- vapply(X = x, FUN=d.log ,  FUN.VALUE = numeric(1))
  
  # Mark the points of the elbow (when the gradient is closest to zero)
  
  lines(abline(v=x[which.min(abs(grads.cube - 0))], col=col1, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  ...
  # Also plot the gradients as a separate graph
  plot(   x=x, y=grads.cube, col=col1, pch=1, ylab="Gradient", 
          ylim=c(0,0.0001))
          #ylim=c(min(grads.cube, grads.sqrt, grads.log),max(grads.cube, grads.sqrt, grads.log)))
  points( x=x, y=grads.sqrt, col=col2, pch=1)
  points( x=x, y=grads.log , col=col3, pch=1)
  
  #lines(abline(v=x[which.min(abs(grads.cube - 0))], col=col1, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  #lines(abline(v=x[which.min(abs(grads.sqrt - 0))], col=col2, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  #lines(abline(v=x[which.min(abs(grads.log  - 0))], col=col3, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  
  legend("topright", legend=c("cube", "sqrt", "log"), pch="o", col=c(col1,col2,col3))
  
}

```

Log model seems to do very well, but using the minimum gradient doesn't get the elbow.

Delve into the gradients of the log model and its derivatives in more detail:
...
See what happens to the Global S Index as the relative size of the base and test datasets change. Method as follows:
 ...
 2. Sample 50:50
 3. Run the S test
 4. Change ratio and repeat
 5. Graph s against proportion
 
```{r testS.Samples, eval=FALSE}
par(mfrow=c(1,1))
#van.poly <- readOGR("./data-raw/vancouver_all/areas.shp")

#bner <- BNER[BNER$YEAR==2016,]
bner <- BNER
proportion <- seq(from=0.99, to=0.01, by=-0.05)
# Do four tests per proportion
prop <- c()
for (i in 1:4) {
...
proportion <- prop
rm(prop)
proportion <- sort(proportion)

s.objects <- mclapply(X=proportion, FUN=function(p) {
  
  samp <- sample(1:nrow(bner), size=nrow(bner)*p)
  #base <- bner[samp, ]
  base <- bner
  test <- bner[samp,]
  #stopifnot( (nrow(base) + nrow(test)) == nrow(bner) )
...
plot(  x=proportion, y=unlist(globalS), main="BNER ALL", col=1, xlab="proportion", ylab="S")

polygon(c(rev(proportion), proportion), c(rev(preds[ ,3]), preds[ ,2]), col = 1, border = NA)
...
lines(x=proportion, y=predict(fit),        col=1, lty=1)
