...
#  msea(base.data, test.data, N=150, n.shifts=5, ignore.zeros = x, step=2)
#})
#r <- tmp[[1]]
#r.withzeros <- tmp[[2]]
#rm(tmp)
...
```{r graphError, fig.width=9, fig.height=15 }
...
par(mfrow=c(2,4))
DEGREE <- 2
USE.LM = TRUE # Whether to use lm or loess
...
calc.gradients <- function(m, x) {
  ...
  d <- Deriv( f = testfun, x="x" ) # Calculate the derivitives
...
# Num cells 1
x <- r.withzeros$num.cells[1:length(r$num.cells)]
y <- r.withzeros$globalS.robust[1:length(r$num.cells)]
lo <- if (USE.LM) lm(y ~ x + I(x^2) ) else loess(y~x, degree = DEGREE)
plot(x,y, main="NumCells (Robust)" )
lines(x=x, y=predict(lo), col='red', lwd=2)
gradients <- calc.gradients(lo,x)
lines(abline(v=x[which.min(abs(gradients - mean(gradients)))], col="red", lty=2)) # Mean gradient
lines(abline(v=x[which.min(abs(gradients - median(gradients)))], col="blue", lty=2)) # Median


# Num cells 2
x <- r.withzeros$num.cells[100:length(r$num.cells)]
...
# Area 1
x <- r.withzeros$cell.areas[1:length(r$num.cells)]
y <- r.withzeros$globalS.robust[1:length(r$num.cells)]
lo <- if (USE.LM) lm(y ~ x + I(x^2) ) else loess(y~x, degree = DEGREE)
plot(x,y, main="cell areas (Robust)")
lines(x=x, y=predict(lo), col='red', lwd=2)
gradients <- calc.gradients(lo,x)
lines(abline(v=x[which.min(abs(gradients - mean(gradients)))], col="red", lty=2)) # Mean gradient
lines(abline(v=x[which.min(abs(gradients - median(gradients)))], col="blue", lty=2)) # Median
...
# Area 1
x <- r$cell.areas[1:length(r$num.cells)]
y <- r$globalS.robust[1:length(r$num.cells)]
lo <- if (USE.LM) lm(y ~ x + I(x^2) ) else loess(y~x, degree = DEGREE)
plot(x,y, main="cell areas (Normal)")
lines(x=x, y=predict(lo), col='red', lwd=2)
gradients <- calc.gradients(lo,x)
lines(abline(v=x[which.min(abs(gradients - mean(gradients)))], col="red", lty=2)) # Mean gradient
lines(abline(v=x[which.min(abs(gradients - median(gradients)))], col="blue", lty=2)) # Median
...
# Area 2
x <- r$cell.areas[100:length(r$num.cells)]
y <- r$globalS.robust[100:length(r$num.cells)]
lo <- if (USE.LM) lm(y ~ x + I(x^2) ) else loess(y~x, degree = DEGREE)
plot(x,y , main="cell areas (Normal)")
lines(x=x, y=predict(lo), col='red', lwd=2)
...
lines(abline(v=x[which.min(abs(gradients - mean(gradients)))], col="red", lty=2))
