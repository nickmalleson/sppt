...
msea <- function(points1, points2, N=20, n.shifts = 10, ignore.zeros=FALSE, step=1, 
                 return.sobject=TRUE, return.grids=TRUE, sppt_func = sppt, ... ) {
  ...
  run.resolution <- function(resolutions) {
    ...
    for (i in resolutions) {
      ...
      for (j in 1:n.shifts) {
        ...
        # Store this result by appending it to the end of the list of results that we have so far
  ...
  pout <- mclapply(X=iterations, FUN= run.resolution )
  #browser()
...
tmp <- mclapply(X = all, FUN = function(x) { # Do all crime types at the same time.
  msea(x[['base']], x[['test']], N=N, n.shifts=N.SHIFTS, ignore.zeros = F, step=STEP, 
...
tmp <- mclapply(X = all, FUN = function(x) { # Do all crime types at the same time.
  msea(x[['base']], x[['test']], N=N, n.shifts=N.SHIFTS, ignore.zeros = F, step=STEP, 
       return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt_diff 
  ) }, mc.preschedule=FALSE, mc.allow.recursive=TRUE)
stopifnot(length(tmp)==4)
# Store the results in the lists in an item called 'r'
for (i in 1:length(tmp)) {
...
Also run again once for BNER and TOB to do the fuzzy mapping. These results wont be used in the graph, so don't need to calculate the index as many times 
...
bner.result <- msea(points1 = p1, points2 = p2,
                    N=N, n.shifts=N.SHIFTS, ignore.zeros = F, step=STEP,
                    return.sobject=TRUE,
                    return.grids=TRUE,
                    sppt_func = sppt_boot # the kind of sppt function to run
)
p1 <- TOB[TOB$YEAR==2015,]
p2 <- TOB[TOB$YEAR==2016,]
tob.result <- msea(points1 = p1, points2 = p2,
                    N=N, n.shifts=N.SHIFTS, ignore.zeros = F, step=STEP,
                    return.sobject=TRUE,
                    return.grids=TRUE,
                    sppt_func = sppt_boot # the kind of sppt function to run
...
test.result.normal <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F, 
                           step=10, return.sobject=FALSE, return.grids=TRUE, sppt_func = sppt)
test.result.boot   <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F, 
                           step=10, return.sobject=FALSE, return.grids=TRUE, sppt_func = sppt_boot)
test.result.diff   <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F, 
                           step=10, return.sobject=FALSE, return.grids=TRUE, sppt_func = sppt_diff)
```

Plot the results of the large iteration test:

```{r runMSEA-test-plot, fig.width=5, fig.height=11}
...
```{r s.model-all-errors, fig.width=9, fig.height=9 }
...
Above we computed the S Index using the 'normal' method, as well as the new 'bootstrap' (`sppt_boot`) and 'proportional difference' (`sppt_diff`). Compare the results for each crime type now.

```{r s.index-comparisons1, fig.width=8, fig.height=5 }
...
cols  = c("#fc8d59","#333333","#99d594") # Colours used later
...
for (i in 1:length(crime.types)) { # For each crime type
  ...
  for (j in 1:length(test.results)) { # For each test result
    ...
    if (j==1) {
      plot(x,y, main=paste0(crime.type), xlab="Number of Cells", ylab="Similarity", 
          col=colour, pch=1, ylim=c(0,1))#, xlim=c(0,10000))
      #legend("bottomright", legend = names, col = cols, pch=1)
    } else {
      points(x,y, col=colour, pch=1)
    }
  }
}

# Repeat for square area
for (i in 1:length(crime.types)) { # For each crime type
  crime.type <- crime.types[i]
  for (j in 1:length(test.results)) { # For each test result
    error.type <- names[j]
    ...
    y <- r[[s.type]] # The S Index (either normal, boot, or prop)
    if (j==1) {
      plot(x,y, main=paste0(crime.type), xlab="Square area of cells", ylab="Similarity", 
           col=colour, pch=1, ylim=c(0,1), xlim=c(0,2000000) )#xlim=c(0,80000))
      legend("bottomleft", legend = names, col = cols, pch=1)
    } else {
      points(x,y, col=colour, pch=1)
    }
  }
...
# Tidy up
rm(crime.types,test.results,cols, names, s.type)
