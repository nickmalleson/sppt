...
In the analysis we want to mask cells that aren't over land as these will distort the results. The [Baseline Thematic Mapping Present Land Use Version 1 Spatial Layer](https://catalogue.data.gov.bc.ca/dataset/baseline-thematic-mapping-present-land-use-version-1-spatial-layer) from the [BC Data Catalogue](https://catalogue.data.gov.bc.ca/dataset) has a suitable dataset that can be used for masking. All land with `PLU_LABEL=Urban` will be included. These ares are defined as: 

 - "All compact settlements including built up areas of cities, towns and villages as well as isolated units away from settlements such as manufacturing plants, rail yards and military camps. In most cases residential use will predominate in these areas."
 
I have downloaded the entire BC dataset to: `./data-raw/BTM_PRESENT_LAND_USE_V1_SVW/` and then also made a smaller version around Vancouver called `BTM_PLU_V1_polygon-cut.shp` (R struggles to read the full file)
 
Note that this this excludes Stanley Park, which will mean that some theft of bike / vehicle crimes are disregarded, but the numbers are very small so it wont overly influence the results.

```{r readLandUseData, cache=TRUE}

# Unzip the files into the working directory
ZIPFILENAME = "./data-raw/BTM_PRESENT_LAND_USE_V1_SVW.zip"
zipfile <- unzip(ZIPFILENAME)

# Read the shapefile
bc.land.use <- readOGR(dsn="./BTM_PRESENT_LAND_USE_V1_SVW/", layer = "BTM_PLU_V1_polygon-cut")

# Delete the extracted file
unlink(zipfile)
unlink("BTM_PRESENT_LAND_USE_V1_SVW", recursive = TRUE) # a left over directory

# Extract the urban land features (these mark the areas that will be included in the analysis)
bc.urban <- bc.land.use[bc.land.use$PLU_LABEL=="Urban",]

# Project it
bc.urban <- spTransform(bc.urban, CRSobj = CRS(proj4string(all.crime)))
proj4string(bc.urban) <- proj4string(all.crime)

## Get rid of areas that don't intersect any crimes (mostly for efficiency). Use st_intersects
## (https://gis.stackexchange.com/questions/245136/how-to-subset-point-data-by-outside-of-polygon-data-in-r)
## All polygons that intersect a point (if not the the output from st_intersects is an integer(0))
#b = sapply(st_intersects(bc.urban, all.crime),function(x){length(x)>0}) 
## Subset polygons
#bc.urban[b] <- bc.urban[b]

rm(ZIPFILENAME, bc.land.use)
```

## Areas data

Finally it is useful to have some administrative boundaries. These aren't used in the analysis, but are good for displaying results.
  ...
  run.resolution <- function(resolutions) {
    ...
    for (i in resolutions) {
      ...
      for (j in 1:n.shifts) {
        ...
        if (!is.null(mask)) {
          #browser()
          # Join spdf to bc.urban. Returns rows for all spdf, which will be NA if there was no join
          a <- over(spdf, mask)
          # Check some cells overlap, otherwise something has almost certainly gone wrong
          stopifnot(length(which(!is.na(a$PLU_LABEL))  ) > 0 )
          # Remove those that have NA for the PLU_LABEL (could have chosen any column)
          spdf <- spdf[which(!is.na(a$PLU_LABEL)),]
          rm(a)
        }
...
# (for testing )
#x <- msea(data1[["base"]], data1[["test"]], N=5, n.shifts=1, step=1, mask=bc.urban,
#          ignore.zeros = F, return.sobject=FALSE,return.grids=FALSE, 
...
tmp <- lapply(X = all, FUN = function(x) { # No longer do these in parallel as the method itself has been parallelised
  msea(x[['base']], x[['test']], N=N, n.shifts=N.SHIFTS, mask=bc.urban,
...
bner.result.largesample <- 
  msea(p1, p2, N=N, n.shifts=N.SHIFTS, mask=bc.urban, ignore.zeros = F, step=STEP, 
       return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt_diff, adj = "none" # (note additional argument to sppt_diff)
       )
...
bner.result.diff <- msea(points1 = p1, points2 = p2,
                    N=N, n.shifts=N.SHIFTS, mask=bc.urban, ignore.zeros = F, step=STEP,
...
And run yet again to test on a very large number of iterations

```{r runMSEA-test, cache=TRUE}
p1 <- BNER[BNER$YEAR==2015,]
p2 <- BNER[BNER$YEAR==2016,]
test.result.normal <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F, mask=bc.urban,
                           step=10, return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt)
test.result.boot   <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F, mask=bc.urban,
                           step=10, return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt_boot)
test.result.diff   <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F, mask=bc.urban,
                           step=10, return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt_diff, adj = "none")

save( test.result.normal, test.result.boot, test.result.diff, file="./large_iteration_test.RData")
rm(p1, p2) # tidy up
```

Plot the results of the large iteration test:

```{r runMSEA-test-plot, fig.width=11, fig.height=15}
...
for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
     ...
     plot(x,y, main=paste0("BNER, ",e,", NORM" ), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}
for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
     ...
     ylim <- if (e=="globalS" | e=="globalS.robust") c(0,1) else NULL
     plot(x,y, main=paste0("BNER, ",e,", BOOT"), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}
for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
     ...
     ylim <- if (e=="globalS" | e=="globalS.robust") c(0,1) else NULL
     plot(x,y, main=paste0("BNER, ",e,", DIFF"), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}

# Get rid of these so that they're not in the main RData file. Too big
rm(test.result.normal, test.result.boot, test.result.diff)
```
...
### Comparing Burglary with Large Sample Size

We ran a test (using `sppt_diff`) with *all* the burglary data (half as base, the other half as test). See how that compares to the normal burglary test (using 2015 and 2016 data).


```{r compare.sample.sizes, fig.width=9, fig.height=5 }
#colours  <- brewer.pal(2, "Set3") # Colours for the points
#s.type = "globalS.robust" # Whether to use robust or non-robust

s.type = "globalS"
plot(x=bner.result.largesample[["num.cells"]] , 
     y=bner.result.largesample[[s.type]] , 
     main="Large sample comparison", xlab="Number of Cells", ylab="S Index", ylim=c(0,1), xlim=c(0,2000), 
     col=1, pch=1)

points(x=all.diff[[1]]$r[["num.cells"]] , 
       y=all.diff[[1]]$r[[s.type]] , 
     col=2, pch=2)


legend("bottomright", legend = c("Large sample", "Normal sample"), pch=c(1,2), col=c(1,2))
...
First, sanity check that the results from these smaller BNER and TOB checks are comparable to the original ones. They should be, obviously, although the BNER and TOB ones are less comprehensive (they ran for many fewer iterations and shifts). The folowing graphs should look similar

```{r sanity.check.bner.results }
#Note that all.diff[[1]] is the BNER results and all.diff[[4]] is the TOB
a <- data.frame("numcells"=all.diff[[1]]$r[["num.cells"]],  
                "sindex.bner1" = all.diff[[1]]$r[["globalS"]], 
                "sindex.tob1" = all.diff[[4]]$r[["globalS"]])
b <- data.frame("numcells"=bner.result.diff[["num.cells"]], "sindex.bner2"= bner.result.diff[["globalS"]])
c <- data.frame("numcells"=tob.result.diff[["num.cells"]],  "sindex.tob2" = tob.result.diff[["globalS"]])

m <- base::merge(x=a, y=b, all.x=TRUE, by="numcells")
m <- base::merge(x=m, y=c, all.x=TRUE, by="numcells")

par(mfrow=c(1,2))
plot(  x=m$numcells, y=m$sindex.bner1, xlim=c(0,4000), main="BNER Sanity check\nshould be the same")
points(m$numcells, m$sindex.bner2, col="blue")
plot(  m$numcells, m$sindex.tob1, xlim=c(0,4000), main="TOB Sanity check\nshould be the same")
points(m$numcells, m$sindex.tob2, col="blue")
rm(a,b,c,m)
```
...
par(mfrow=c(2,length(crime.types))) # 2 rows, one for num.cells and square.area, 4 columns (one for each crime type)
do.s.plot("num.cells", xlim=c(0,2000), text.size=1.5)
...
par(mfrow=c(2,length(crime.types))) # 2 rows, one for num.cells and square.area, 4 columns (one for each crime type)
do.s.plot("num.cells", xlim=c(0,500), text.size = 2.5)
do.s.plot("cell.areas", xlim=c(0,250000), text.size = 2.5)
```
...
```{r changepoints-burgd.example, fig.width=9, fig.height=9 }

# Burglary

# penalized in ShapeChange
bner.changepoint.1 <- changept(bner.result.diff[["globalS"]] ~ tp(bner.result.diff[["num.cells"]], sh = 1), 
                             pnt = TRUE, ci = TRUE, param = FALSE) 

# non-penalized in ShapeChange
bner.changepoint.2 <- changept(bner.result.diff[["globalS"]] ~ tp(bner.result.diff[["num.cells"]], sh = 1), 
                             ci = TRUE, param = FALSE)

# TOB
tob.changepoint.1 <-  changept(tob.result.diff[["globalS"]] ~ tp(tob.result.diff[["num.cells"]], sh = 1), 
                             pnt = TRUE, ci = TRUE, param = FALSE) 

# non-penalized in ShapeChange
tob.changepoint.2 <-  changept(tob.result.diff[["globalS"]] ~ tp(tob.result.diff[["num.cells"]], sh = 1), 
                             ci = TRUE, param = FALSE) 


par(mfrow=c(2,2))
plot(bner.changepoint.1)
plot(bner.changepoint.2)
plot(tob.changepoint.1)
plot(tob.changepoint.2)

```

XXXX HERE Now try again but using the larger tests
...
_This has probably now been depricated by the above analysis that uses StepChange to identify the point at which the error changes with resolution_
...
```{r s.model-all-log, fig.width=11, fig.height=11 }

range01 <- function(x){(x-min(x))/(max(x)-min(x))} # (https://stackoverflow.com/questions/5665599/range-standardization-0-to-1-in-r#5665644)
...
for (a in all) {
  ...
  alpha <- coef(m.log)[1]
  beta <-  coef(m.log)[2]
  FUNC.log <- function(x) alpha + beta*log(x)
  ...
  # Try to find the 'elbow'
  elbow  <- x[min(which(grads.log1 < 0.00002))]
  elbow2  <- x[min(which(grads.log1 < 0.00001))]
  elbow3 <- x[min(which(grads.log1 < 0.0001))]
  elbow4 <- x[min(which(grads.log1 < 0.001))]

  # Plot the original points
  plot(  x, y, main=name, col="#999999", pch='o', #ylim=c(0,1)
       xlab="Number of Cells", ylab="Global S Index")
  #abline(v=x[which.min(abs(grads.log1 - 1))], col="blue", lty=2, lwd=2) # Point at which the gradient is closest to 0
  
  # Add the model
  lines(x=x, y=predict(m.log ), col=1, lwd=2)
  
  # Add horizonatal and vertical lines at the point that the first derivative is equal to beta (this is the 'elbow')
  #abline(v=beta, col="red")
  
  # Add vertical line at the elbow when beta becomes very small
  #abline(v=elbow,  col="red")
  abline(v=elbow,  col="red")
  abline(v=elbow2,  col="green")
  abline(v=elbow3,  col="blue")
  abline(v=elbow4,  col="orange")
  
  # Add the equation to the graph
  #mtext(paste0("alpha = ",round(alpha,3),"\nbeta = ",round(beta,3) ), side=4, line=-8, las=2)
  mtext(paste0("y = ",round(alpha,3)," + ",round(beta,3),"*ln(x)","\n",
               "elbow"," = ", round(elbow,5)
               ), side=4, line=-15, las=2)
  

  
  # Also plot the derivatives as a separate graph
...
It looks like the point at which the change in S-Index is < 0.00002 successfully captures the 'elbow' (the red line). Redo the charts above to make this clearer.

Begin by calculating the elbow and some other useful things for each crime

```{r calc_model_and_elbow}

FORM.LOG <- y ~ log(x) # A formula and function for log(x)

ELBOW_THRESHOLD <- 0.00002

for (i in 1:length(all)) {
  r     <- all[[i]]$r   # This is the results of the msea test
  name <- all[[i]]$name # This is the name of the crime type
  #base <- all[[i]]$base # Base and test points (not actually needed here)
  #test <- all[[i]]$test
  
  x <- r$num.cells[START:END]
  y <- r$globalS[START:END]
  x.sqm <- r$cell.areas[START:END] # Square area of each cell (not used in the analysis)
  ...
  # Differentiate the function (1st derivative)
  d.log1 <- Deriv( f = FUNC.log,  x="x", nderiv = 1 )
  grads.log1 <- vapply(X = x, FUN=d.log1, FUN.VALUE = numeric(1))
  
  # Find the 'elbow' and store this as part of the list of all crimes
  elbox.index <- min(which(grads.log1 < ELBOW_THRESHOLD)) # The index at which the elbow crosses the x axis
  elbow       <- x[elbox.index] # The elbow itself (x axis value; number of cells)
  elbow.sqm   <- x.sqm[elbox.index] # The elbow in square area of cell units (rather than total number of cells)
  
  # The point at which the elbow hits the y axis
  elbow.y <- FUNC.log(elbow)
  ...
  all[[i]]$elbox.index <- elbox.index
  all[[i]]$elbow <- elbow
  all[[i]]$elbow.sqm <- elbow.sqm
  all[[i]]$elbow.y <- elbow.y
...
Make a table of the different elbow values using the current threshold of: `r ELBOW_THRESHOLD`.

```{r model_and_elbow-table, results="asis"}

elbow.table <- data.frame(
    "Crime Type" = vapply(X = all, FUN.VALUE = character(1), FUN=function(a) a$name),
    "Elbow - Number of Cells)" = vapply(X = all, FUN.VALUE = integer(1), FUN=function(a) as.integer(round(a$elbow) ) ) ,
    "Elbow - Square Area)" = vapply(X = all, FUN.VALUE = integer(1), FUN=function(a) as.integer(round(a$elbow.sqm) ) )
    )

xtable(elbow.table, type="html")

```
A bar chart to make the table easier to perceive

```{r model_and_elbow-barchart }
barplot(height = elbow.table$Elbow...Square.Area., names.arg = elbow.table$Crime.Type, 
        ylab="Elbow (square area of cells)", main = "Elbow values for different crime types" )

# A pdf for the paper
pdf(file="./figs_for_paper/model_and_elbow-barchart.pdf", width=7, height=4)
barplot(height = elbow.table$Elbow...Square.Area., names.arg = elbow.table$Crime.Type, 
        ylab="Square area of cells")
dev.off()
```

Finally the full graph with tangents etc.

```{r model_and_elbow-graph, fig.width=9, fig.height=7 }

options(scipen=1) # A bit of scientific notation is OK
par(mfrow=c(length(all)/2, length(all)/2))

do.plot <- function(i) {
  a <- all[[i]]
  name <- a$name # This is the name of the crime type
  # Plot the original points
  plot(  a$x, a$y, main=name, col="#999999", pch='o', #ylim=c(0,1)
       xlab="Number of Cells", ylab="Global S Index")
 
  # Plot the tangent 
  yint <- a$elbow.y - (a$d.log1(a$elbow)*a$elbow) # Y intercept of the tangent
  abline(a = yint, b = a$d.log1(a$elbow), lty="dashed", lwd=2) # The gradient is  d.log1(elbow)
  
  # Add the model
  lines(x=x, y=predict(a$m.log), col=1, lwd=2)
  
  # Add vertical and horizontal line at the elbow and a point
  abline(v=a$elbow,   col="red", lty="dashed")
  abline(h=a$elbow.y, col="red", lty="dashed")
  points(x=a$elbow, y=a$elbow.y, col="blue", pch=16, cex=2)
  
  # Add the equation to the graph
  mtext(paste0("y = ",round(a$alpha,3)," + ",round(a$beta,3),"*ln(x)","\n",
               "elbow (x)"," = ", round(a$elbow,5),"\n",
               "elbow (y)"," = ", round(a$elbow.y,5)
               ), side=4, line=-15, las=2, cex=0.8)
...
# A pdf for the paper
pdf(file="./figs_for_paper/model_and_elbow.pdf", width=9, height=7)
par(mfrow=c(length(all)/2, length(all)/2))
for (i in 1:length(all)) {
  do.plot(i)
}
...
pdf(file="./figs_for_paper/example_error_graph_for_paper.pdf", width=9, height=7)
do.plot(4)
dev.off()
