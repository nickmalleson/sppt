...
XXXX - check with Martin what is happenning here.

The best results seem to be when we use a function with degree 3 to define the curve, but then disregard the x^3 term when calculating the gradient.
...
```{r , eval=FALSE}
par(mfrow=c(1,2))
#x <- r$num.cells[1:length(r$num.cells)]
#y <- r$globalS.robust[1:length(r$num.cells)]

#x <- r.withzeros$cell.areas[100:length(r$num.cells)]
#y <- r.withzeros$globalS.robust[100:length(r$num.cells)]

#x <- r$num.cells[100:length(r$num.cells)]
#y <- r$globalS.robust[100:length(r$num.cells)]

x <- r$num.cells[1:length(r$num.cells)]
y <- r$globalS.robust[1:length(r$num.cells)]

lo <- if (USE.LM) lm(y ~ x + I(x^2) ) else loess(y~x, degree = DEGREE)

lo <- lm(y ~ x + I(x^2) )

gradients <- calc.gradients(lo,x)

plot(x,y, main="NumCells (Normal)" )
lines(x=x, y=predict(lo), col='red', lwd=2)




# Find the x value with the optimal gradient

lines(abline(v=x[which.min(abs(gradients - 0))], col="blue", lty=2))
lines(abline(h=y[which.min(abs(gradients - 0))], col="blue", lty=2))

#testfun <- function(x) x + x^2
plot(x=x, y=gradients)
