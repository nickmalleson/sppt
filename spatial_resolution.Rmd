...
msea <- function(points1, points2, N=20, n.shifts = 10, ignore.zeros=FALSE, step=1, 
  ...
  # The pout list has one item for each resolution. Merge these so that there is one long
  # list with all the results for all resolutions
  
  
  # Extract those separate parts into their own variables to make it easier to see what's going on
  
  # Get each component from the results that came from each resolution
...
tmp <- mclapply(X = all, FUN = function(x) {
  msea(x[['base']], x[['test']], N=50, n.shifts=1, ignore.zeros = F, step=5, 
...
(_Not running, takes ages, do it outside of this Rmd_)
...
test.result <- msea(points1 = p1, points2 = p2,
                    ...
                    N=100, n.shifts=5, ignore.zeros = F, step=5,
                    return.sobject=TRUE,
                    return.grids=TRUE)
```
...
```{r runMSEA-test-plot, cache=TRUE, eval=FALSE}
...
_TODO_: Need to work out why the RSS and RMSE values decrease with the number of cells. They measure error, so I'd expect them to increase. Unless the larger number of 0s artifically reduce them.
