...
msea <- function(points1, points2, N=20, n.shifts = 10, ignore.zeros=FALSE) {
  ...
  for (i in seq(1,N)) {
    ...
    for (shift in 1:n.shifts) {
      ...
      bb2 <- bb # The new bounding box
      bb2["coords.x1","min"] <- bb["coords.x1","min"] - ( cell.width / 2  ) # Min x gets smaller
      bb2["coords.x2","min"] <- bb["coords.x2","min"] - ( cell.height / 2 ) # Min y gets smaller
      bb2["coords.x1","max"] <- bb["coords.x1","max"] + ( cell.width / 2  ) # Max x gets larger
      bb2["coords.x2","max"] <- bb["coords.x2","max"] + ( cell.height / 2 ) # Max y gets larger
      
      # Calculate the centre of the lower-left cell (the one with the smallest coordinates)
      centre.x <- bb2[1,1] + ( cell.width / 2 )
      centre.y <- bb2[2,1] + ( cell.height / 2 )
...
```{r runMSEA}
base.data <- BNER[BNER$YEAR==2016,]
test.data <- BNER[BNER$YEAR==2015,]

# Do for both with and without zeros, at the same time

tmp <- mclapply(X = c(T,F), FUN = function(x) {
  msea(base.data, test.data, N=50, ignore.zeros = x)
})
