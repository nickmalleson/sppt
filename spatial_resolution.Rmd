...
#no_cores <- detectCores() / 3  # Detect the number of cores that are available and use half (often CPUs simulate 2 threads per core)
...
bner.result.diff <- msea(points1 = p1, points2 = p2,
                    ...
                    sppt_func = sppt_diff # the kind of sppt function to run
)
...
Above we computed the S Index using the 'normal' method, as well as the new 'bootstrap' (`sppt_boot`) and 'proportional difference' (`sppt_diff`). Compare the results for each crime type now using the non-robust index.

Also mark on the graphs the points at which I will draw the maps below

```{r s.index-comparisons1, fig.width=12, fig.height=7 }

ITERATIONS.TO.MAP <- c(20, 50, 80, 150, 200, 300)
...
colours  <- brewer.pal(3, "Accent") # Colours for the points
...
do.s.plot <- function(x.axis.type, s.type = "globalS", xlim=NULL, iter.to.map = ITERATIONS.TO.MAP, text.size=1.2) {
  ...
  for (i in 1:length(crime.types)) { # For each crime type
    ...
    for (j in 1:length(test.results)) { # For each test result
      ...
      for (iter in iter.to.map) {
        ...
        abline(v = xval, col=line.colours[col.count], lty="dashed")
        text  (x = xval, y=0.2, labels=paste0(iter,"\n",signif(xval, 4)), cex=text.size)
...
Repeat with much more narrow x axes. 
...
par(mfrow=c(2,length(crime.types))) # 2 rows, one for num.cells and square.area, 4 columns (one for each crime type)
do.s.plot("num.cells", xlim=c(0,400), text.size = 2.0)
do.s.plot("cell.areas", xlim=c(0,100000), text.size = 2.0)
```
...
# Code is slightly complicated because the BNER and TOB results aren't as comprehensive. This is because
# I don't generate full grid results for every resolution because it takes up too much memory. The code
# below finds the most suitable results from those that are available (i.e. the nearest to the ones listed
# in ITERATIONS.TO.MAP).
...
indices.to.map <- sapply(X=ITERATIONS.TO.MAP, FUN=function(x) {min(which(bner.result.diff$num.cells - x > 0))})
