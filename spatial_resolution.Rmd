...
```{r s.model, fig.width=9, fig.height=7 }
par(mfrow=c(2,4))

START <- 20 # Sometimes ignore first few grids
...
calc.gradients <- function(m, x) {
  f <- function(x) coef(m)[1] + (coef(m)[2]*x) + (coef(m)[3]*(x^2))
  stopifnot(! (FALSE %in% predict(m) == f(x))) # Check function has been specified correctly
  d <- Deriv( f = f, x="x" ) # Calculate the derivitives
...
make.gradient.plot( x=r.withzeros$num.cells, 
                    y=r.withzeros$globalS.robust,
  form=FORM, main = "NumCells : Robust :\nZeros" )

make.gradient.plot( x=r.withzeros$num.cells, 
                    y=r.withzeros$globalS,
  form=FORM, main = "NumCells : Normal :\nZeros" )

make.gradient.plot( x=r.withzeros$cell.areas, 
                    y=r.withzeros$globalS.robust,
...
make.gradient.plot( x=r$num.cells, 
                    y=r$globalS.robust,
  form=FORM, main = "NumCells : Robust :\nNo Zeros" )
...
make.gradient.plot( x=r$cell.areas, 
                    y=r$globalS.robust,
  form=FORM, main = "CellAreas : Robust :\nNo Zeros" )
