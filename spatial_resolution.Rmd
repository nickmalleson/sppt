...
Log model seems to do very well, but using the miniumu gradient doesn't get the elbow.

Delve into the gradients of the log model in more detail:

```{r s.model-all3, fig.width=9, fig.height=9 }
...
proportion <- seq(from=0.99, to=0.01, by=-0.01)
...
globalS <- mclapply(X=proportion, FUN=function(p) {
...
plot(x=proportion, y=unlist(globalS))
