...
no_cores <- detectCores() - 1 # Detect the number of cores that are available and use all but one
...
START <- 1 # Sometimes ignore first few grids
...
_XXXX - check with Martin what is happenning here._
_The best results seem to be when we use a function with degree 3 to define the curve, but then disregard the x^3 term when calculating the gradient._
...
for (a in all) {
  ...
  plot(   x=x, y=grads.normal1, col="red", pch=1, ylab="Gradient", ylim=c(-0.05,0.05))
...
for (a in all) {
  ...
  lines(x=x, y=predict(m.quad), col="red", lwd=2)
  lines(x=x, y=predict(m.cube), col="blue", lwd=2)
  ...
  # Also plot the gradients as a separate graph
  plot(   x=x, y=grads.quad,  col=1, pch=1, ylab="Gradient", ylim=c(-0.0005,0.0005))
  ...
  legend("topright", legend=c("Quad","Cube","Cube2","Weird"), pch="o", col=c(1,2,3,4))
  
}

```

Repeat again, but this time do 

```{r s.model-all2, fig.width=9, fig.height=6 }
par(mfrow=c(length(all)/2,4))
FORM1 <- y ~ x + I(x^2) + I(x^3) # Cubic formula to fit to change in S index
FORM2 <- y ~ x + I(x^2) # Quadratic formula to fit to change in S index
...
for (a in all) {
  ...
  # Plot points
  plot(  x, y.normal, main=name, pch=1, ylim=c(0,1), col="black", 
         xlab="Number of Cells", ylab="Global S Index")
  ...
  lines(abline(v=x[which.min(abs(grads.normal1 - 0))], col=col1, lty=2, lwd=2)) # Point at which the gradient is closest to 0
  lines(abline(v=x[which.min(abs(grads.normal2 - 0))], col=col2, lty=2, lwd=2)) # Point at which the gradient is closest to 0

  # Also plot the gradients as a separate graph
  plot(   x=x, y=grads.normal1, col=col1, pch=1, ylab="Gradient", ylim=c(-0.0005,0.0005))
  points( x=x, y=grads.normal2, col=col2, pch=1)
  #points( x=x, y=grads.normal1, col=col1, pch=1)
  lines(abline(v=x[which.min(abs(grads.normal1 - 0))], col=col1, lty=2, lwd=2))
  lines(abline(v=x[which.min(abs(grads.normal2 - 0))], col=col2, lty=2, lwd=2))
...
Graph those results using XXXX DECIDE ON WHICH DATA TO USE - ROBUST? and WITH ZOROS? 
