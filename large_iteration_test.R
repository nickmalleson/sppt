...
test.result.normal <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F,
                           step=10, return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt)
test.result.boot   <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F,
                           step=10, return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt_boot)
test.result.diff   <- msea(points1 = p1, points2 = p2, N=500, n.shifts=1, ignore.zeros = F,
                           step=10, return.sobject=FALSE, return.grids=FALSE, sppt_func = sppt_diff)

save.image(file="./large_iteration_test.RData")

pdf(file="./large_iteration_test.pdf", width=11, height=11)


error.stats <- c("rss", "r.squared", "rmse", "globalS", "globalS.robust") # The error stats. to use
par(mfrow=c(6,length(error.stats))) # One plot for each error statistic using both num.cells and square area

#r <- test.result$results # This is the results of the msea test

for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
    x <- test.result.normal[[type]]
    y <- test.result.normal[[e]]
    ylim <- if (e=="globalS" | e=="globalS.robust") c(0,1) else NULL
    plot(x,y, main=paste0(name,", ",e,", NORM" ), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}
for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
    x <- test.result.boot[[type]]
    y <- test.result.boot[[e]]
    ylim <- if (e=="globalS" | e=="globalS.robust") c(0,1) else NULL
    plot(x,y, main=paste0(name,", ",e,", BOOT"), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}
for (type in c("num.cells", "cell.areas")) {
  for (e in error.stats) {
    x <- test.result.diff[[type]]
    y <- test.result.diff[[e]]
    ylim <- if (e=="globalS" | e=="globalS.robust") c(0,1) else NULL
    plot(x,y, main=paste0(name,", ",e,", DIFF"), xlab=type, ylab=paste("Error (",e,")"), ylim=ylim)
  }
}
