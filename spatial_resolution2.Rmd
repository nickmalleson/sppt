...
Quick test: if we do the analysis with more points, we should see the elbow occur later, and higher Global S values

```{r tempTest, cache=TRUE}
...
do.call("grid.arrange", c(plots, ncol=len(errors.temp)))
rm(errors.temp)
...
do.call("grid.arrange", c(plots, ncol=len(error.stats)))
...
```{r s.index-ecca_graphs1, fig.width=7, fig.height=7 }
...
```{r s.index-ecca_graphs-withci, fig.width=7, fig.height=7 }
...
```{r s.index-ecca_graphs-sameplot, fig.width=10, fig.height=7 }
