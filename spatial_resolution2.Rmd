...
=======
library(ggplot2)
>>>>>>> a7cfed73bea5022fe35277d456da824b2e4dd5b9
...
tmp <- lapply(X = all, FUN = function(x) { 
  msea(x[['base']], x[['test']], N=N, n.shifts=N.SHIFTS, mask=vancouver.boundaries,
       ignore.zeros = F, step=STEP, return.sobject=FALSE, return.grids=FALSE, 
...
# Delete old files, including some shapefiles that are created  later in chunk map.iterations
...
Quick test: doesif we do the analysis with more points, we should see the elbow occur later, and higher Global S values
...
temp.bner.diff.small <- msea(
  points1 = BNER[BNER$YEAR==2015,], 
  points2 = BNER[BNER$YEAR==2016,],
  N=40, n.shifts=S, mask=vancouver.boundaries, ignore.zeros = F, step=STEP, return.sobject=TRUE, return.grids=TRUE,
  sppt_func = sppt_diff, adj = "none" # (note additional argument to sppt_diff)
  )
gc()
temp.bner.diff.large <- msea(
  points1 = BNER[BNER$YEAR==2016 | BNER$YEAR==2014 | BNER$YEAR==2012 | BNER$YEAR==2010 | BNER$YEAR==2008,],
  points2 = BNER[BNER$YEAR==2015 | BNER$YEAR==2013 | BNER$YEAR==2011 | BNER$YEAR==2009 | BNER$YEAR==2007,],
  N=40, n.shifts=S, mask=vancouver.boundaries, ignore.zeros = F, step=STEP, return.sobject=TRUE, return.grids=TRUE,
  sppt_func = sppt_diff, adj = "none" # (note additional argument to sppt_diff)
  )
gc()
...
# Small (few)
plot(  temp.bner.diff.small$num.cells, temp.bner.diff.small$globalS.robust, ylim=c(0,1), main="Few points - DIFF")
points(temp.bner.diff.small$num.cells, temp.bner.diff.small$globalS, ylim=c(0,1), col="red")
legend("bottomright", legend=c("globalS.robust", "globalS"), pch='o', col=c("black", "red"))

# Large (many)
plot(  temp.bner.diff.large$num.cells, temp.bner.diff.large$globalS.robust, ylim=c(0,1), main="Many points - DIFF")
points(temp.bner.diff.large$num.cells, temp.bner.diff.large$globalS, ylim=c(0,1), col="red")
legend("bottomright", legend=c("globalS.robust", "globalS"), pch='o', col=c("black", "red"))

# NORMAL

plot(  temp.bner.norm.small$num.cells, temp.bner.norm.small$globalS.robust, ylim=c(0,1), main="Few points - Normal")
points(temp.bner.norm.small$num.cells, temp.bner.norm.small$globalS, ylim=c(0,1), col="red")
legend("bottomright", legend=c("globalS.robust", "globalS"), pch='o', col=c("black", "red"))

# Large (many)
plot(  temp.bner.norm.large$num.cells, temp.bner.norm.large$globalS.robust, ylim=c(0,1), main="Many points - Normal")
points(temp.bner.norm.large$num.cells, temp.bner.norm.large$globalS, ylim=c(0,1), col="red")
legend("bottomright", legend=c("globalS.robust", "globalS"), pch='o', col=c("black", "red"))

#write.csv(cbind(temp.bner.diff.large$iteration, temp.bner.diff.large$num.cells, temp.bner.diff.large$globalS, temp.bner.diff.small$globalS), file="~/Desktop/temp.csv")

#write.csv(temp.bner.diff.large$results[[5]]@data, file="~/Desktop/large_05.csv" )
#write.csv(temp.bner.diff.large$results[[39]]@data, file="~/Desktop/large_39.csv" )
#write.csv(temp.bner.diff.small$results[[5]]@data, file="~/Desktop/small_05.csv" )
#write.csv(temp.bner.diff.small$results[[39]]@data, file="~/Desktop/small_39.csv" )
...
# Large (many)
plot(  temp.bner.diff.large$num.cells, temp.bner.diff.large$globalS, ylim=c(0.7,1), main="Many points - DIFF", pch="o", cex=0.5)
points(temp.bner.diff.small$num.cells, temp.bner.diff.small$globalS, ylim=c(0,1), col="red", pch="o", cex=0.5)
legend("bottomright", legend=c("many points", "few points"), col=c("black", "red"), pch="o", cex=1)

```
=======
For info, graph those errors.

```{r graph.bner.result, fig.width=10, fig.height=10 }
errors.temp <- c("rss", "r.squared", "rmse", "globalS", "globalS.robust", "mean.similarity", "mean.similarity.robust") # The error stats. to use
...
for (e in errors.temp) {
  x <- r$num.cells
  y <- r[[e]]
  the.plot <- ggplot(data = data.frame("x"=x, "y"=y ), mapping = aes(x,y)) +
...
options(scipen=1) # A bit of scientific notation is OK
error.stats <- c("rss", "r.squared", "rmse", "globalS", "globalS.robust", "mean.similarity", "mean.similarity.robust") # The error stats. to use
...
for (a in all.diff) {
  ...
  for (e in error.stats) {
    if (is.null(r[[e]])) {
      plot(1,1, col="red", main=paste0("Error. Statistic:\n'",e, "' not found"))
    } else {
      x <- r$num.cells
      y <- r[[e]]
...
Repeat, fitting a smooth curve with confidence intervals.

```{r s.model-all-errors2, fig.width=11, fig.height=11}
options(scipen=1) # A bit of scientific notation is OK
plots = list()
plot.count <- 1

for (i in 1:length(all.diff)) {
...
<<<<<<< HEAD
### Graphs for ECCA - examining Global S Index changes

Graph each crime as a separate plot

```{r s.index-ecca_graphs1, fig.width=7, fig.height=4 }
...
par(mfrow=c(2,2)) # One plot for each crime type
...
for (i in 1:length(crime.types)) { # For each crime type
  ...
  plot(x,y, main=paste0(crime.type), xlab="Number of Cells", ylab="Similarity",  ylim=c(0,1), xlim=c(0,1000))
}

```

Try again with confidence intervals

```{r s.index-ecca_graphs-withci, fig.width=7, fig.height=4 }

# Make some lists so that the same plotting code can loop 
crime.types = vapply(X=all, FUN=function(x)x$name, FUN.VALUE = character(1)) # The crime types (4)

colours  <- brewer.pal(4, "Set2") # Colours for the points
...
for (i in 1:length(crime.types)) { # For each crime type
  ...
  x <- r[["num.cells"]] 
  y <- r[[s.type]] # The S Index (either normal or robust)
  df = data.frame("NumCells"=r[["num.cells"]], "Similarity"=r[[s.type]] )
  
  plots[[i]] <- ggplot(df, aes(NumCells, Similarity)) +
    #geom_hex(bins=20) +
    geom_point(size=2) + 
    #guides(colour=guide_legend(title="Sensor")) + # Change the title of the legend
    #geom_smooth(se=TRUE, level=0.99)
    geom_smooth(method="loess", se=TRUE, level=0.99, color=colours[i])
...
```{r s.index-ecca_graphs-sameplot, fig.width=7, fig.height=4 }
...
par(mfrow=c(1,1)) # One plot for each crime type
...
for (i in 1:length(crime.types)) { # For each crime type
  ...
  y <- r[[s.type]] # The S Index (either normal or robust)
  if (i==1) {
    plot(x,y, main="Global S Index by Crime Type", xlab="Cell Area (ha)", ylab="Similarity",  
           xlim=c(0,1000), ylim=c(0.5,1), col=colours[i], pch=i, cex=2, xaxt='n')
    #axis(1, at=x, labels=r[["num.cells"]])
    axis(1, at=x, labels=round(r[["cell.areas"]]/10000) )
      
  } else {
    points(x,y, col=colours[i], pch=i)
  }
  legend("topright", legend = crime.types,  pch=1:length(crime.types), col = colours, cex=2)
...
### Comparing Global S Index results from different methods (sppt, sppt_diff, sppt_boot)
=======
...
for (index in indices.to.map) {
  ...
  sq.area <- bner.result.diff[["cell.areas"]][index] / 10000 # hectares
...
```{r map.localS, fig.width=8, fig.height=6 }
statistic <- "localS" # The statistic to map

par(mfrow=c(2,2))

for (i in 4:1) {
  index <- round(length(bner.result.diff$results) / i )
  
  # Create the shading manually
  s <- list("breaks"=c(-0.5, 0, 0.5), "cols"=c("#fc8d59","#ffffbf", "#ffffbf","#99d594"))
  
  choropleth(bner.result.diff$results[[index]], bner.result.diff$results[[index]]@data[,statistic], main=paste(statistic,i),
             lty=0, shading = s)
...
Now do _similarity_ (calculated by sppt_diff)


Map the Local S for a few grids, just out of interest

```{r map.similarity, fig.width=8, fig.height=6 }
statistic <- "similarity" # The statistic to map
...
results.to.map <- NA
if (length(bner.result.diff$results) > 50) {
  results.to.map <- sample(x=1:length(bner.result.diff$results), size=50, replace = FALSE )
  print("Only mapping 50 of all grids ")
} else {
  results.to.map <- 1:(length(bner.result.diff$results)-1)
}

par(mfrow=c(1,1))
last.result <- bner.result.diff$results[[length(bner.result.diff$results)]] # Convenience for list result generated (the smallest grid)
...
for ( index in results.to.map) {
  choropleth( bner.result.diff$results[[index]], bner.result.diff$results[[index]]@data[,statistic], shading = s, lty=0, add=TRUE )
}
