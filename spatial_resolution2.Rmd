    ...
    for (i in resolutions) {
      ...
      for (j in 1:n.shifts) {
        ...
        if ( isTRUE(all.equal(sppt_func, sppt_diff))) { # Check using sppt_diff
          parallel.output[["mean.similarity"]]        <- c(parallel.output[["similarity"]],        mean( ( spdf@data$similarity) ) )
          parallel.output[["mean.similarity.robust"]] <- c(parallel.output[["similarity.robust"]], mean( ( spdf@data$similarity.robust) ) )
        } else {
          parallel.output[["mean.similarity"]] <-        NA
          parallel.output[["mean.similarity.robust"]] <- NA
        }
